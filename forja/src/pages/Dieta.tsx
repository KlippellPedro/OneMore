...
import { usePerfil, useMapaAlimentos, usePlanos, useAguaDoDia } from '../state/hooks'
import { totalDoDia, macrosDe, somaMacros, ZERO } from '../lib/nutricao'
import { registrarAlimento, removerRegistro, checarMetasDoDia, addAgua } from '../lib/acoes'
...
import type { Alimento, RegistroDieta } from '../db/types'
  ...
  return (
    <div>
        ...
        <Card className="p-4 mb-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] uppercase tracking-wider text-muted font-semibold">Meta {n0(perfil.metaKcal)} kcal</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Macro nome="Carboidrato" atual={total.carb} meta={perfil.metaCarb} cor="var(--color-warn)" />
            <Macro nome="Gordura" atual={total.gord} meta={perfil.metaGord} cor="#c084fc" />
          </div>
        </Card>
        ...
        <Card className="p-3.5 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Barra valor={(agua?.ml ?? 0) / perfil.metaAgua} cor="#4dabf7" altura={6} />
            </div>
            ...
            <div key={nome} className="mb-4">
              <div className="flex items-baseline justify-between mb-2 px-1">
                ...
                <span className="text-[11.5px] text-muted tabular-nums">
                  {itens.length ? `${n0(m.kcal)} kcal - P ${n0(m.prot)}g` : plano?.horario ?? ''}
                </span>
              </div>
                    ...
                    <button key={r.id} onClick={() => setDetalhe(r)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-muted">
                          ...
                          {' - P '}{n1(mm.prot)}g C {n1(mm.carb)}g G {n1(mm.gord)}g
                        </p>
                      </div>
                ...
                <div className="flex">
                  ...
                  {plano && plano.itens.length > 0 && (
                    <button onClick={() => setAplicarPlano(nome)}
                      ...
                      Lancar plano ({plano.itens.length})
                    </button>
                  )}
                </div>
              </Card>
      ...
      <Confirmar aberto={!!aplicarPlano} titulo="Lancar a refeicao do plano?"
        texto="Todos os alimentos planejados para essa refeicao entram no diario de hoje."
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-1">
        <span className="text-[10.5px] text-muted font-semibold uppercase tracking-wide">{nome}</span>
      </div>
